...
while max_fitness != zzz:
    ...
    for i in range(demes):
        for j in range(ind_per_island):
            ...
            deme_dict = dict(zip(total_population[i],fitness[i]))
            father = roulette_wheel_pop(total_population[i],
                                        get_probability_list(deme_dict), 1)
            print(father)                            
            father_index = total_population[i].index(str(father[0]))


            deme_selected = random.randint(0, demes - 1)
            deme_dict = dict(
                zip(total_population[deme_selected],
                    fitness[deme_selected]))
            mother = roulette_wheel_pop(total_population[deme_selected],
                                        get_probability_list(deme_dict), 1)
            mother_index = total_population[deme_selected].index(str(mother[0]))
            replace = deme_selected = random.randint(0, 1)
            if replace ==1:
                total_population[i][father_index] = crossover(father, mother)
            else:
                total_population[deme_selected][mother_index] = crossover(father,mother)
        


    mutated = mutation(total_population)
