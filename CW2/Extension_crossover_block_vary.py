...
def zip_fitness_eval(population, R_noise,shuffled):
    maxi_fitness = 0
    fitness = []
    gene_i = 0
    gene_j = 0
    for i in range(demes):
        for j in range(ind_per_island):
            ...
            for k in range(2*number_of_sites):
                if shuffled[k] == 'i' and population[i][j][k] == '1':
                    gene_i +=1
                elif shuffled[k] == 'j' and population[i][j][k] == '1':
                    gene_j+=1
            fitness_equation = R_noise[gene_i - 1][gene_j - 1] * (2**gene_i +
                                                                  2**gene_j)
            fitness.append(fitness_equation)
...
while simulation != 10:
    generation = 0
    R_noise = Rij()
    fokf = ['i','j']*number_of_sites
    # print(fokf)
    genetic_definiton = sorted(fokf, key=lambda k: random.random())
    ...
    while max_fitness != zzz and generation <= 1500:
        fitness, max_fitness = zip_fitness_eval(total_population, R_noise, genetic_definiton)
        ...
        for i in range(demes):
            for j in range(ind_per_island):
                if j == 0:
                    ...
                    father = roulette_wheel_pop(
                        total_population[deme_selected],
